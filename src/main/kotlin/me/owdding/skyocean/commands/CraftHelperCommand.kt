...
object CraftHelperCommand {
    ...
    fun registerCommands(event: RegisterSkyOceanCommandEvent) {
        event.register("recipe") {
            ...
            thenCallback("skyshards") { selectSkyshardsRecipe() }
    ...
    private fun setRecipeAmount(amount: Int) {
        if (amount <= 0) {
    ...
    private fun selectSkyshardsRecipe() {
        ...
        try {
            ...
            data?.let {
                ...
                val containsCycle = list.any { e -> e is SkyShardsCycleElement }
    ...
    private fun selectRecipe(input: String) {
        var amount = 1
    ...
    private fun customRecipeAdd(input: String) {
        var amount = 1
        val item = SkyBlockId.fromName(input, dropLast = false) ?: SkyBlockId.unknownType(input) ?: run {
            val splitName = input.substringBeforeLast(" ")
            amount = input.substringAfterLast(" ").toIntOrNull() ?: 1
    ...
    private fun customRecipeSet(input: String) {
        var amount = 1
        val item = SkyBlockId.fromName(input, dropLast = false) ?: SkyBlockId.unknownType(input) ?: run {
            val splitName = input.substringBeforeLast(" ")
            amount = input.substringAfterLast(" ").toIntOrNull() ?: 1
    ...
    private fun customRecipeRemove(input: String) {
        val item = SkyBlockId.fromName(input, dropLast = false) ?: SkyBlockId.unknownType(input) ?: run {
            val splitName = input.substringBeforeLast(" ")
            SkyBlockId.fromName(splitName) ?: SkyBlockId.unknownType(splitName)
        }
