...
object CoinRainOverlay : SkyOceanOverlay() {
    ...
    fun headBlueprintFromTexture(texture: String): ItemStackBlueprint { // this should maybe be moved somewhere else?
        ...
        val profile = GameProfile(UUID.nameUUIDFromBytes(texture.toByteArray()), "", propertyMap)
        stack.set(DataComponents.PROFILE, ResolvableProfile.createResolved(profile))
        return ItemStackBlueprint(stack)
    }

    private val coinRainBlueprints by lazy {
        listOf(
        ...
        ).map { headBlueprintFromTexture(it) }
    }

    private val coinRainItems by lazy { levelBound { coinRainBlueprints.map { it.create() } } }
            ...
            repeat(coinMultiplier) { // this might cause a performance issues at high resolutions, im not sure
                fallingCoinsList.add(
                    FallingCoin(
                        ...
                        item = coinRainItems.getValue().random(),
                    ),
                )
            }
        }
