...
public abstract class BakedGlyphMixin {
    ...
    @Shadow
    protected abstract void render(boolean italic, float x, float y, Matrix4f pose, VertexConsumer buffer, int color, boolean bold, int packedLight);
    ...
    @WrapOperation(
        method = "renderChar",
        at = @At(value = "INVOKE", target = "Lnet/minecraft/client/gui/font/glyphs/BakedGlyph;render(ZFFLorg/joml/Matrix4f;Lcom/mojang/blaze3d/vertex/VertexConsumer;IZI)V", ordinal = 0)
    )
    public void onRenderShadow(
        ...
        if (MiscConfig.INSTANCE.getFullTextShadow()) {
            ...
            original.call(instance, italic, x, y, pose, buffer, color, bold, packedLight);
        }
    }
