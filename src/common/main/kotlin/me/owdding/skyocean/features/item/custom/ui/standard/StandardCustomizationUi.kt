            ...
            LayoutFactory.horizontal(alignment = MIDDLE) {
                vertical {
                    Widgets.button {
                        ...
                        it.withSize(160, 20)
                        val entry = item.getCustomData()?.get(CustomItemDataComponents.MODEL)?.toModelSearchEntry()
                        if (entry != null) {
                            it.withRenderer(ItemSelectorOverlay.resolveRenderer(item, entry, 20))
                        } else {
                            it.withRenderer(ItemSelectorOverlay.resolveRenderer(item, !BuiltInRegistries.ITEM.getKey(item.getItemModel()).path, 20))
                        }
                        it.withCallback {
                            McClient.setScreen(ItemSelectorOverlay(McScreen.self, it, ItemStack(Items.STICK)))
                        }
                    }.add()
                }
                val width = 50
